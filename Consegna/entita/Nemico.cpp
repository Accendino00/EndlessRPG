...
/**
 * @brief Costruttore di nemico.
 * 
 * In base al tipo di nemico, ne inizializza le caratteristiche.
 * Si distingono le due categorie NORMAL_ENEMY e BOSS_ENEMY.
 * 
 * Poi, per ognuno di questi, vi sono tanti tipi di nemici diversi.
 * La scelta di quale far nascere è casuale. Pertanto, per i nemici
 * normali si sceglie un numero intero casuale compreso tra 0 e 5 e
 * si sceglie il rispettivo nemico normale:
 *   - 0: Prillo
 *   - 1: Sputafuoco
 *   - 2: Bombardiere
 *   - 3: Soldato semplice
 ...
 *   - 5: Bruco (verticale)
 * 
 * Vi sono poi due tipi di boss:
 *   - 0: Super Prillo
 *   - 1: Alternatore
 * 
 * Per più dettagli sui vari nemici, vedere la descrizione dell'idea
 * all'interno della funzione.
 * 
 * I nemici scalano le loro caratteristiche in base al livello e alla
 * difficoltà. 
 * 
 * @param type NORMAL_ENEMY, BOSS_ENEMY
 * @param posy L'angolo in alto a sinistra dove si trova il nemico
 * @param posx L'angolo in alto a sinistra dove si trova il nemico
 * @param tipoStanza Serve a decidere il colore del nemico e dei proiettili (che coincidono)
 * @param numLivello Serve a far scalare le caratteristiche del nemico
 */
Nemico::Nemico (int type, int posy, int posx, int tipoStanza, int numLivello) {
    ...
    scaleFactor = scaleFactor + (numLivello*0.5);
    ...
    if(type == NORMAL_ENEMY) {
        ...
        switch(idNemico) {
////////////// Prillo (2x2)
            // idea: si gira e spara in ogni direzione, mostrando da dove sta per sparare
            case 0:
            ...
            break;
////////////// Sputafuoco (1x1)
            // idea: si muove in una direzione e spara nella stessa dierzione (+/- 45 gradi); quando arriva ad un muro (accessibile da falso) si gira e ripete
            case 1:
                // Statistiche del nemico delineate dallo scale factor calcolato in precedenza
...
/**
 * @brief Fa le azioni specifiche del nemico all'interno della sua
 * action list, in base a dove si trova il giocatore e alla stanza dove è.
 * 
 * In partioclare, si può muovere in diversi modi, sparare in diversi modi 
 * o cambiare frame.
 * 
 * @param stanza 
 * @param player 
...
/**
 * @brief Fa sparare un proiettile nella direzione indicata.
 * 
 * offset è un valore che indica quanto il proiettile deve essere spostato rispetto alla direzione.
 * Se offset è 0, il proiettile viene sparato nella direzione indicata.
 * Se offset è 1, il proiettile viene sparato nella direzione indicata, ma spostato di 1 a "destra",
 * se è -1, viene spostato di 1 a "sinistra".
 * 
 * @param dir 
 * @param offset 
 * @param stanza 
...
/**
 * @brief Stampa il nemico. Se è il boss, allora
 * stampa dall'inizio anche la sua vita.
 * 
 * @param offsetY 
 * @param offsetX 
 */
...
void Nemico::stampaVita (int blocchi) {
    ...
    /* 
        ...
        Il numero di blocchi è il parametro "blocchi" in input.
